...
T0=$(date +%s)
#
SRCDATADIR="$(cd $HOME/.. ; pwd)/data/DrugCentral/SI_Sets"
#
DBNAME="drugcentral"
SCHEMA="public"
#
...
psql -d $DBNAME -c "COMMENT ON COLUMN structures.status IS 'Added by $(basename $0) for OrangeBook sets (OFP, ONP, OFM) from 2020 paper.'"
###
...
while [ "$i" -lt "$N" ]; do
	...
	s=$(echo "$line" |awk -F '\t' '{print $1}'|sed 's/ //g')
