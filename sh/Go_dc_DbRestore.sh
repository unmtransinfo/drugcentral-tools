...
sudo -u "$PGSU" psql -d $DBNAME -c "GRANT SELECT ON ALL TABLES IN SCHEMA public TO jjyang"
sudo -u "$PGSU" psql -d $DBNAME -c "GRANT SELECT ON ALL SEQUENCES IN SCHEMA public TO jjyang"
sudo -u "$PGSU" psql -d $DBNAME -c "GRANT EXECUTE ON ALL FUNCTIONS IN SCHEMA public TO jjyang"
###
gunzip -c "$dumpfile" |psql -q "$DBNAME"
#
