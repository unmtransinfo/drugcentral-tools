...
/**	Represents one DrugDB product, defined by ID, comprised of 1+ 
	ingredients, and associated data.

	In this version all ingredients assumed to be active.
*/

public class DrugDBProduct
	implements Comparable<Object>
{
  private HashMap<Integer,DrugDBIngredient> ingredients;
  ...
  private DrugDBProduct() {} //disallow default constructor

  public DrugDBProduct(Integer _id)
  {
    this.id = _id;
    ingredients = new HashMap<Integer,DrugDBIngredient>();
  }
  ...
  public void addIngredient(DrugDBIngredient _igt)
  {
    this.ingredients.put(_igt.getID(),_igt);
    this.n_ingredient = Math.max(this.n_ingredient,this.ingredientCount());
  }
  public DrugDBIngredient getIngredient(Integer _iid) { return this.ingredients.get(_iid); }
  public boolean hasIngredient(Integer _iid) { return this.ingredients.containsKey(_iid); }
  public Collection<DrugDBIngredient> getIngredients() { return this.ingredients.values(); }
  ...
  public String getMixtureSmiles()
  {
    ...
    for (DrugDBIngredient ingr: this.getIngredients())
    {
  ...
  {
    return (id>((DrugDBProduct)o).id ? 1 : (id<((DrugDBProduct)o).id ? -1 : 0));
  }
}
