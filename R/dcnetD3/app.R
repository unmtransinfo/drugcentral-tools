...
### networkDS is a R package for network visualization, using D3.js.
### 
...
ui <- fluidPage(
  ...
  fluidRow(
    column(2, 
           ...
           sliderInput('opacity', 'Opacity', min = 0.0, max = 1.0, value = 0.8)
           ),
    column(8, 
  ...
  fluidRow(
    column(12, HTML("Web app build with R-shiny, networkD3, D3.js, and rcdk."))
  )
)
...
server <- function(input, output, session) {
  ...
  output$force <- renderForceNetwork({
    ...
    output$cpds <- renderDataTable(drugs, options = list(pageLength=5, pagingType="simple", searching=F, info=T))
    output$tgts_sp <- renderDataTable(tgts_sp, options = list(pageLength=5, pagingType="simple", searching=F, info=T))
    output$tgts_pc <- renderDataTable(tgts_pc, options = list(pageLength=5, pagingType="simple", searching=F, info=T))
    output$prots_pc <- renderDataTable(prots_pc, options = list(pageLength=5, pagingType="simple", searching=F, info=T))
    
    ###Convert data to igraph nodes and edges.
    ...
    nodes <- data.frame(name = node_ids)
    ...
    edges <- data.frame(from = edge_froms, to = edge_tos, name = edge_ids)
    edges$title <- edge_titles
    edges$length <- edge_lengths

    message(sprintf("DEBUG: edges = %d", nrow(edges)))

    #title = paste("DrugCentral drug-target network:", input$qry)
    ...
    #m <- membership(g)
    m <- c(rep(1, nrow(drugs)), rep(2, nrow(tgts_sp)))
    if (n_tgt_pc>0)
    {
      m <- c(m, c(rep(3, nrow(tgts_pc)), rep(4, nrow(prots_pc))))
    }

    g_d3 <- igraph_to_networkD3(g, group = m)
